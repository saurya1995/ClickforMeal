...
$cart=mysql_query("SELECT * FROM cart where email='$email'", $MyConnection) or die(mysql_error());
...
$prodname=$row['name'];
$prodprice=$row['price'];	
$order=mysql_query("INSERT INTO `ecommerce`.`order` (`email`, `product_id`, `name`, `quantity`, `price`, `img`) VALUES('$email','$pi','$prodname','$pqty', '$prodprice','$pimg')", $MyConnection) or die(mysql_error());

}

$un=mysql_query("DELETE FROM cart WHERE email='$email' and product_id='$pi'", $MyConnection) or die(mysql_error());
